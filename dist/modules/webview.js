...
const ManifestBuilder_1 = require("./Html/ManifestBuilder");
const updateIcon_1 = require("./Html/updateIcon");
const updateAppName_1 = require("./Html/updateAppName");
const createIfNotExist_1 = require("./Html/createIfNotExist");
const ProgressBar_1 = require("./Html/ProgressBar");
const getJavaVersion_1 = require("./Html/getJavaVersion");
const checkForUpdates_1 = require("./Html/checkForUpdates");
const updateAppTheme_1 = require("./Html/updateAppTheme");
const updateApkVersion_1 = require("./Html/updateApkVersion");
...
class Webview {
    ...
    installModule(env, context) {
        ...
        //@ts-ignore
        env.sdk = this.sdk;
        checkForUpdates_1.getUpdate(env);
    ...
    ///  - update app name
    create() {
        let examplesFolder = createIfNotExist_1.createIfNotExist(path.join(this.env.builder.cache, '..', 'examples'));
        ...
        const exampleRepoDownloadLink = GitListDir_1.getDownloadLink(this.example.user, this.example.repo);
        ...
        if (!fs.existsSync(exampleFolderPath) || this.env.force) {
            ...
            downloadsdk({
                url: GitListDir_1.getDownloadLink(this.example.user, this.example.repo),
    ...
    downloadSDK(callback, force = false) {
        ...
        if (!force && fs.existsSync(sdkFolder)) {
            ...
            downloadsdk({
                url: GitListDir_1.getDownloadLink(this.sdk.user, this.sdk.repo),
                repo: this.sdk.repo,
    ...
    updateFiles(callback) {
        ...
        // updating icon
        updateIcon_1.updateIcon(this.env, this);
        // updating app name
        updateAppName_1.updateAppName(this.env, this);
        // updating app version
        updateApkVersion_1.updateApkVersion(this.env, this);
        // generate Android Manifest file
        const manifestFileData = ManifestBuilder_1.getManifest(this.env, this, permissions, deep_links, screenOrientation);
    ...
    rebuildApk(callback) {
        const progress = new ProgressBar_1.LoadingBar();
        const apkToolLink = GitListDir_1.getFileDownloadLink(this.apk_tool.user, this.apk_tool.repo, this.apk_tool.dir, this.apk_tool.file);
        const apkToolFilePath = path.join(this.env.builder.dir, 'build_tools', this.apk_tool.file);
        const apkSignerLink = GitListDir_1.getFileDownloadLink(this.apk_signer.user, this.apk_signer.repo, this.apk_signer.dir, this.apk_signer.file);
        ...
        const proc = child_process_1.spawn('java', args_, { cwd: cacheFolderPath });
        let log = "";
    ...
    build() {
        ...
        // check Java version
        getJavaVersion_1.javaVersion((error, home) => {
            if (error) {
                ...
                this.downloadSDK(() => {
                    ...
                    this.updateFiles((error) => {
                        updateAppTheme_1.updateTheme(this.env, (err) => {
                            this.rebuildApk((error) => {
                                this.sign(() => {
                                    try {
                                        ...
                                        checkForUpdates_1.getUpdateMessage();
                                    }
                                    catch (e) {
    ...
    sign(callback) {
        const progress = new ProgressBar_1.LoadingBar();
        let apksigner_path = path.join(this.env.builder.dir, 'build_tools', this.apk_signer.file);
        ...
        let args_ = ['-jar', apksigner_path, '--apks', apk_file_path];
        ...
        // @ts-ignore
        const proc = child_process_1.spawn('java', args_, { cwd: build_working_dir });
        proc.stdout.on('data', data => {
            if (progress.isRunning === false) {
...
function downloadsdk(args, callback) {
    args.retry -= 1;
    let state = {
        data: 0,
        moveCursor: false,
        ...
        progress: new ProgressBar_1.LoadingBar()
    };
    request
